...
// let string = 'minhazul abedin mithu';
const flag = 'z';
const esc = 'm';
...
//const stuffedString = byteStuffing(string);

function byteDeStuffing(inputStr) {
    const stringLength = inputStr.length;
    for (let i = 0; i < stringLength; i++) {
        if(inputStr[i] == esc && (inputStr[i+1] == esc || inputStr[i+1] == flag)){
            inputStr.splice(i, 1);
        }
    }
    inputStr =  inputStr.join('');
...
// const deStuffedString = byteDeStuffing(stuffedString);
